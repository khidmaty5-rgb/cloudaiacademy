...
function getAdminAppWithCert(): App | null {
  const name = 'adminAppCertificatesUploadPdf';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.length) return NextResponse.json({ error: 'Empty PDF body' }, { status: 400 });
