...
function getAdminApp(): App {
  const name = 'adminAppJournalPresignReplace';
...
export async function POST(
  ...
  try {
    ...
    if (!key || typeof key !== 'string' || !key.startsWith('journal/articles/')) {
      return NextResponse.json({ error: 'PDF path not configured' }, { status: 400 });
    }
